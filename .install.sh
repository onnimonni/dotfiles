...
echo "Then create a new ssh key named 'github' in Secretive and run:"
echo "$ ssh-add -L | grep github@secretive > ~/.ssh/github_secretive.pub"
...
echo "The same key signs commits, so register it as a signing key too:"
echo "$ gh ssh-key add --type signing ~/.ssh/github_secretive.pub"
echo "And to verify your own signatures locally:"
echo "$ printf '%s %s\\n' \"\$(git config user.email)\" \"\$(cat ~/.ssh/github_secretive.pub)\" > ~/.ssh/allowed_signers"
